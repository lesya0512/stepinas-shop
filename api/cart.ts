import { IAddTovarToCartFx, ICartItem, IDeleteCartItemsFx, IUpdateCartItemCountFx } from "@/types/cart";
...
export const addTovarToCartFx = createEffect(
    async ({ jwt, setSpinner, ...dataFields }: IAddTovarToCartFx) => {
        try {
            setSpinner(true)
            ...
            if (data?.error) {
                const newData: { newCartItem: ICartItem } = await handleJWTError(
                    data.error.name, 
                    {
                        repeatRequestMethodName: 'addTovarToCartFx',
